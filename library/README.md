...
- `bubbles_light/`（**主线推荐**）：日系清新手绘细黑钢笔描边 + 内部填充纯净暖黄色（`#F6A83C`），视觉轻盈优雅，文字搭配深棕色（`#4A2800`）。
- `bubbles_yellow/`：浓郁粗黑手绘钢笔描边 + 内部填充亮黄色（`#F6A83C`），适合极度张扬的高冲突场景。
- `bubbles/`：纯黑白手绘镂空框（内部实心纯白），适合黑白极简排版。
...
在项目的 `layout/blockN.json` 中直接作为 `asset` 元素引用（支持宽高自由缩放）：
